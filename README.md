...
A：因为 Origin Request 处理后的响应体不能超过 1.33M，因此如果处理后的图片大于这一体积，就不会在当前请求中返回处理后的图片，而是使用重定向的方式来重新发起请求，让其命中 BOS 来获取图片。这次重定向行为在响应头中通过 Cache-Control 禁止了 CloudFront 缓存，但是 CloudFront 默认的最小缓存时长为 1s，因此会出现在重定向行为发生后又多次重定向（缓存存活中），而在重定向 3~6 次后正常返回图片（重定向缓存被销毁）。可以按照如下方式修改：
